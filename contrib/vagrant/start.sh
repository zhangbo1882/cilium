...
function write_cilium_cfg() {
    ...
    cilium_options=" --debug --pprof --enable-hubble --hubble-listen-address :4244 --enable-k8s-event-handover --k8s-require-ipv4-pod-cidr --auto-direct-node-routes --enable-host-firewall --enable-endpoint-routes --devices=enp0s3,enp0s8 --kube-proxy-replacement=strict"
