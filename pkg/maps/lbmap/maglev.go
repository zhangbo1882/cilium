...
func InitMaglevMaps(ipv4, ipv6 bool, tableSize uint64) error {
	...
	if ipv4 {
		name := MaglevOuter4MapName(tableSize)
		MaglevOuter4Map, err = NewMaglevOuterMap(name, MaxEntries, tableSize, dummyInnerMapSpec)
		if err != nil {
			return err
		}
	}
	if ipv6 {
		name := MaglevOuter6MapName(tableSize)
		MaglevOuter6Map, err = NewMaglevOuterMap(name, MaxEntries, tableSize, dummyInnerMapSpec)
		if err != nil {
			return err
		}
	}
...
// maglev BPF maps by probing their table size from the map's names.
func MaybeInitMaglevMapsByProbingTableSize() (uint64, error) {
...
func GetOpenMaglevMaps() map[string]*maglevOuterMap {
	...
	case MaglevOuter4Map != nil:
		maps[MaglevOuter4MapName(MaglevOuter4Map.TableSize)] = MaglevOuter4Map
	case MaglevOuter6Map != nil:
		maps[MaglevOuter6MapName(MaglevOuter6Map.TableSize)] = MaglevOuter6Map
	}
